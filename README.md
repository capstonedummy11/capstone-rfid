...
- Passed: student portal UI alignment check on 2026-07-06: `npx prettier --check` for touched student portal UI files and `git diff --check`.
- Blocked: `npm run build` for the student portal UI alignment pass could not reach Vue compilation because Wayfinder runs `php artisan`, and this shell resolves PHP 7.4.25 while Composer dependencies require PHP >= 8.4.0.
- Passed: student Online Classes Rekognition warning removal check on 2026-07-06: `npx prettier --check resources/js/pages/StudentParent/OnlineClasses.vue`, `git diff --check`, and text search confirmed the unavailable-Rekognition warning remains only on admin pages.
- Passed: instructor dashboard targeted frontend checks on 2026-07-06: `npx prettier --check resources/js/pages/Auth/Admin/Dashboard.vue` and `git diff --check`.
- Blocked: `npm run build` for the instructor dashboard pass could not reach Vue compilation because Wayfinder runs `php artisan`, and this shell resolves PHP 7.4.25 while Composer dependencies require PHP >= 8.4.0.
- Passed: instructor dashboard Used Labs removal check on 2026-07-06: `npx prettier --check resources/js/pages/Auth/Admin/Dashboard.vue` and `git diff --check`.
...
| 2026-07-06 | Make the instructor dashboard proper and working. | `app/Http/Controllers/DashboardController.php`, `routes/web.php`, `resources/js/pages/Auth/Admin/Dashboard.vue`, `README.md` | Replaced the static dummy admin/instructor dashboard with a controller-backed dashboard. Instructor users now see scoped schedule, student, attendance, online class, laboratory, and unread message data, while admin users see whole-system dashboard totals and tables. |
...
| 2026-07-06 | Fix student portal UI alignment, especially the dashboard, and use a bell icon for Notifications in the nav. | `resources/js/pages/StudentParent/Dashboard.vue`, `resources/js/pages/StudentParent/Attendance.vue`, `resources/js/pages/StudentParent/ExcuseLetters.vue`, `resources/js/pages/StudentParent/Messages.vue`, `resources/js/pages/StudentParent/Notifications.vue`, `resources/js/pages/StudentParent/OnlineClasses.vue`, `resources/js/pages/StudentParent/Profile.vue`, `resources/js/layouts/AuthNavbar.vue`, `resources/js/components/StudentPortal/LinkedStudentSelector.vue`, `resources/js/components/StudentPortal/StatCard.vue`, `resources/css/app.css`, `README.md` | Added shared student portal page/shell styling for consistent alignment, refined dashboard header/stat/filter responsiveness, cleaned visible separator text, made linked-student selector and stat cards more stable, and changed student portal nav icons so Notifications uses a bell instead of matching Messages. |
