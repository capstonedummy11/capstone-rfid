...
| Excuse letters       | Create, attach files, Parent signature/approval, generated signed PDF, protected Messenger delivery, and one-time recipient-Instructor approve/deny decisions with editable Student/Parent result email. | Delivery remains synchronous rather than queued; deployments should monitor mail latency/failures. | `StudentsController`, `MessageController`, portal/Messenger pages, mail transport, workflow tests. |
