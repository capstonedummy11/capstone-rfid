                ...
                >
                    <div>
                        ...
                        <p class="text-sm text-slate-500">
                            Manage sections with strand alignment.
                        </p>
                    </div>
                    ...
                    <table class="w-full table-auto border-collapse">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="border border-gray-300 px-4 py-3">
                                    {{ section.strand_code ?? 'N/A' }}
                                </td>
                                <td class="border border-gray-300 px-4 py-3">
                                    {{ getYearLabel(section.year_level) }}
                                </td>
                                <td class="border border-gray-300 px-4 py-3">
                                    {{ section.semester }}
                                </td>
                                ...
                                <td class="border border-gray-300 px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            v-if="!section.is_writable"
                                            class="rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-500"
                                            >Locked:
                                            {{
                                                section.academic_year_status
                                            }}</span
                                        >
                                    </div>
                                </td>
...
const selectedAcademicYear = ref(
    props.filters.academic_year ??
        (props.activeAcademicYearId ? String(props.activeAcademicYearId) : ''),
);
...
const filteredSections = computed<Section[]>(() => {
    return (props.sections as Section[]).filter((section) => {
        ...
        const matchesStrand =
            ...
            String(section.strand_id) === selectedStrand.value;
        const matchesYear =
            selectedYear.value === '' ||
        ...
        const matchesSemester =
            ...
            section.semester === selectedSemester.value;
...
const schoolYearOptions = computed(() => {
    return props.academicYearOptions.map((year) => year.name).filter(Boolean);
});
const currentAcademicYearOption = computed(() =>
    props.academicYearOptions.find(
        (year) =>
            String(year.academic_year_id) ===
            String(props.activeAcademicYearId),
    ),
);
const selectedSchoolYearOption = computed(() =>
    props.academicYearOptions.find((year) => year.name === form.school_year),
);
const schoolYearWarning = computed(() => {
    const year = selectedSchoolYearOption.value;
    if (!year) return '';
    if (year.status === 'draft') {
        return 'Warning: this academic year is still Draft. Records created here are for advance setup and will not be current until activated.';
    }
    if (year.status === 'closed' || year.status === 'archived') {
        return 'Warning: this academic year is already past/locked. The server may reject changes to protect historical records.';
    }
    return '';
});
...
const openAddModal = () => {
    ...
    form.status = 'active';
    form.school_year =
        currentAcademicYearOption.value?.name ??
        schoolYearOptions.value[0] ??
        '';
    form.semester =
        currentAcademicYearOption.value?.active_semester ?? '1st Semester';
    ...
    if (isEditing.value) {
        form.put(
            route('admin.sections.update', {
                id: selectedSection.value?.section_id,
            }),
            {
                preserveState: true,
                onSuccess: () => {
...
const deleteSection = (section: Section) => {
    ...
    deleteForm.delete(
        route('admin.sections.destroy', { id: section.section_id }),
        {
            preserveState: true,
