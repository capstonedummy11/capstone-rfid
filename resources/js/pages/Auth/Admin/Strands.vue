    ...
    <div class="max-w-[1400px] mx-auto px-4 py-6">
      ...
      <section class="bg-white shadow-lg rounded-lg p-6 mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-slate-500">View, manage, and organize strand records and information.</p>
          </div>
          <div class="flex items-center gap-2">
            <button @click="openAddModal" class="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">Add Strand</button>
          </div>
        </div>
      </section>
      ...
      <section class="bg-white shadow-lg rounded-lg p-6">
        <div class="overflow-x-auto">
          <table class="w-full table-auto border-collapse">
            <thead>
              <tr class="bg-gray-50">
                <th class="border border-gray-300 px-4 py-3 text-left">Strand Code</th>
                <th class="border border-gray-300 px-4 py-3 text-left">Strand Name</th>
            ...
            <tbody>
              <tr v-for="strand in filteredStrands" :key="strand.strand_id" class="hover:bg-gray-50">
                <td class="border border-gray-300 px-4 py-3 font-medium">{{ strand.strand_code }}</td>
                <td class="border border-gray-300 px-4 py-3">{{ strand.strand_name }}</td>
                <td class="border border-gray-300 px-4 py-3">{{ strand.department }}</td>
                ...
                <td class="border border-gray-300 px-4 py-3">
                  <div class="flex items-center gap-2">
                    <button @click="openEditModal(strand)" class="rounded-md bg-indigo-600 px-3 py-1 text-sm text-white hover:bg-indigo-700">Edit</button>
                    <button @click="deleteStrand(strand)" class="rounded-md bg-rose-500 px-3 py-1 text-sm text-white hover:bg-rose-600">Delete</button>
                  </div>
                </td>
              </tr>
            </tbody>
          ...
          <div v-if="filteredStrands.length === 0" class="text-center py-8 text-gray-500">No strand records found.</div>
        </div>
      </section>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div class="w-full max-w-2xl rounded-lg bg-white p-6 max-h-[90vh] overflow-y-auto">
          <h2 class="text-xl font-semibold mb-4">{{ isEditing ? 'Edit Strand' : 'Add New Strand' }}</h2>

          <form @submit.prevent="submitForm" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Strand ID</label>
                <input v-model="form.strand_id" type="text" disabled class="w-full rounded-md border border-slate-300 bg-slate-50 px-3 py-2" />
              </div>
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Strand Code *</label>
                <input
            ...
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Strand Name *</label>
              <input
...
const submitForm = () => {
  ...
  if (isEditing.value) {
    form.put(route('admin.strands.update', { id: selectedStrand.value?.strand_id }), {
      preserveState: true,
      onSuccess: () => {
        closeModal();
...
const deleteStrand = (strand: Strand) => {
  if (!confirm(`Are you sure you want to delete ${strand.strand_code} - ${strand.strand_name}?`)) {
    return;
  }

  const deleteForm = useForm({});
  deleteForm.delete(route('admin.strands.destroy', { id: strand.strand_id }), {
    preserveState: true,
