...
<template>
    <div class="bg-slate-50 px-4 py-8">
        <section class="mx-auto max-w-3xl rounded-md border border-slate-200 bg-white p-5 shadow-sm">
            <div class="border-b border-slate-100 pb-4">
                <h1 class="text-2xl font-bold text-slate-900">Message Instructor</h1>
                <p class="mt-1 text-sm text-slate-500">
                    Send attendance concerns, absence notes, or questions directly to an instructor.
                </p>
                <p
                    v-if="flashSuccess"
            ...
            <form class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2" @submit.prevent="submit">
                <label class="md:col-span-2">
                    <span class="mb-1 block text-xs font-semibold uppercase text-slate-500">Instructor</span>
                    <select
                        v-model="form.instructor_user_id"
                        class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
                        required
                    >
                        <option value="" disabled>Select instructor</option>
                        <option v-for="instructor in instructors" :key="instructor.value" :value="instructor.value">
                            {{ instructor.label }}
                        </option>
                    </select>
                    <p v-if="form.errors.instructor_user_id" class="mt-1 text-xs text-red-600">
                        {{ form.errors.instructor_user_id }}
                    </p>
                </label>

                <label>
                    <span class="mb-1 block text-xs font-semibold uppercase text-slate-500">Sender</span>
                    <select
                ...
                <label>
                    <span class="mb-1 block text-xs font-semibold uppercase text-slate-500">Student Number</span>
                    <input
                ...
                <label class="md:col-span-2">
                    <span class="mb-1 block text-xs font-semibold uppercase text-slate-500">Subject</span>
                    <input
                        v-model="form.subject"
                        type="text"
                        placeholder="Absent status, attendance correction, question"
                        class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
                    />
                </label>

                <label class="md:col-span-2">
                    <span class="mb-1 block text-xs font-semibold uppercase text-slate-500">Message</span>
                    <textarea
                        v-model="form.body"
                        rows="6"
                        class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
                ...
                <label class="md:col-span-2">
                    ...
                    />
                    <p class="mt-1 text-xs text-slate-400">PDF, JPG, PNG, or WEBP up to 5 MB.</p>
                    <p v-if="form.errors.attachment" class="mt-1 text-xs text-red-600">{{ form.errors.attachment }}</p>
                </label>
