...
<template>
    <div class="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
        <div class="flex min-h-16 items-start justify-between gap-3">
            <div class="min-w-0">
                <p class="truncate text-3xl font-black">{{ value }}</p>
                <p class="mt-1 text-xs font-bold text-slate-700">{{ label }}</p>
            </div>
            <component
                :is="icon"
                v-if="icon"
        ...
        <p
            v-if="helper"
            class="mt-3 text-[10px] font-bold text-slate-400 uppercase"
        >
            {{ helper }}
        </p>
    </div>
